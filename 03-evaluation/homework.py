...
def search_function_q3(q):
    ...
    results = vindex_q3.search(
        query_vector=question_vector, filter_dict={"course": q["course"]}, num_results=5
    )
    return results
...
doc_texts_q4_fixed = [
    "search_document: " + doc["question"] + " " + doc["text"] for doc in documents
]
doc_embeddings_q4_fixed = model.encode(doc_texts_q4_fixed, show_progress_bar=True)
print(
    f"Эмбеддинги для документов созданы. Размерность: {doc_embeddings_q4_fixed.shape}"
)
...
client.recreate_collection(
    ...
    vectors_config=models.VectorParams(
        size=doc_embeddings_q4_fixed.shape[1], distance=models.Distance.COSINE
    ),
)
...
client.upload_records(
    ...
    records=[
        models.Record(id=idx, vector=vector.tolist(), payload=doc)
        ...
        query_filter=models.Filter(
            must=[
                models.FieldCondition(
                    key="course", match=models.MatchValue(value=q["course"])
                )
            ]
        ),
        limit=5,
