...
try:
    ...
    logging.info(f"ChromaDB client initialized. Host: {config.CHROMA_HOST}, Port: {config.CHROMA_PORT}")
...
def load_documents_to_chroma(documents: List[Dict]):
    ...
    logging.info(f"Loading {len(documents)} documents into Chroma collection '{config.CHROMA_COLLECTION_NAME}'...")
    
    # Prepare data for batch upsert
    ids = [doc['id'] for doc in documents]
    embeddings = [doc['embedding'].tolist() for doc in documents] # Convert numpy arrays to lists
