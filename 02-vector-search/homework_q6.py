...
for i, doc in enumerate(ml_documents):
    ...
    if text_embedding_list and isinstance(text_embedding_list[0], np.ndarray):
        ...
        print(
            f"Warning: Could not generate embedding for document index {i}, skipping."
        )
...
if points_to_upsert:
    try:
        ...
        print(
            f"Successfully upserted {len(points_to_upsert)} points into '{COLLECTION_NAME}'."
        )
    except Exception as e:
...
if not query_embedding_q1_list or not isinstance(
    query_embedding_q1_list[0], np.ndarray
):
