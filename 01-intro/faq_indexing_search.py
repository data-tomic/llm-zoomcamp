...
for (
    course_in_documents_raw
) in (
    documents_raw
):  # Используем другое имя для переменной цикла, чтобы избежать путаницы
...
for i, doc in enumerate(documents):
    try:
        es_client.index(
            index=index_name, document=doc, id=i
        )  # Используем 'i' как id документа
...
search_query_q4 = {
    ...
    "query": {
        "bool": {
            "must": {
                "multi_match": {
                    ...
                    "fields": [
                        "question^4",
                        "text",
                    ],  # Используем те же поля и буст, что и в Q3 (или можно question^3 как в примере из описания ДЗ)
            ...
            "filter": {"term": {"course": course_filter_q4}},
        }
    },
}
...
if len(response_q4["hits"]["hits"]) >= 3:
    third_question_q4 = response_q4["hits"]["hits"][2]["_source"][
        "question"
    ]  # [2] для третьего результата (индексация с 0)
...
if "hits" in response_q4 and "hits" in response_q4["hits"]:
    for hit in response_q4["hits"]["hits"]:  # Берем все 3 документа, полученные в Q4
        ...
        context_entries.append(
            context_template.format(question=question_text, text=answer_text)
        )
...
try:
    ...
    print(
        "tiktoken library is not installed. Please install it using: pip install tiktoken"
    )
...
if (
    "generated_answer" in locals()
    and "encoding" in locals()
    and "num_tokens_q6" in locals()
):
    ...
    print(
        f"Number of output tokens (from Ollama's response, tokenized for gpt-4o): {num_tokens_ollama_response}"
    )
    ...
    cost_output_specific = (
        num_tokens_ollama_response / 1000
    ) * price_output_per_1k_tokens
    total_cost_specific_request = cost_input_specific + cost_output_specific

    print(
        f"Cost for your specific input ({num_tokens_q6} tokens): ${cost_input_specific:.6f}"
    )
    print(
        f"Cost for Ollama's output ({num_tokens_ollama_response} tokens): ${cost_output_specific:.6f}"
    )
    print(
        f"Total cost for this single specific request (if it were OpenAI gpt-4o): ${total_cost_specific_request:.6f}"
    )

    # Расчет стоимости для 1000 таких же запросов
    total_cost_1000_specific_requests = total_cost_specific_request * 1000
    print(
        f"Total estimated cost for 1000 such specific requests: ${total_cost_1000_specific_requests:.2f}"
    )

else:
    print(
        "Could not perform specific cost calculation: 'generated_answer', 'encoding', or 'num_tokens_q6' not found."
    )
...
print(
    f"Total cost for 1000 requests (average 150 input, 250 output tokens): ${total_cost_1000_avg_requests:.2f}"
)
